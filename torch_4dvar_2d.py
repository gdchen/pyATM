...
import logging
from neural_model.differ_module import Order2_Diff1_2D_X, Order2_Diff1_2D_Y, Order2_Diff2_2D_X, Order2_Diff2_2D_Y
from neural_model.kernel_module import GaussianSmoothing
...
from tools.plot_helper import generate_image_movement_gif
...
class Args:
    ...
    load_model = False
    save_model = True
    main_folder = "da_2d"   # 问题定义
    sub_folder = "shallowwave_observe_study"             # 主方案定义
    prefix = "short_noobs"
    checkpoint_file_name = "./checkpoint/ode/" + prefix + "test1.pth.tar"
    save_checkpoint_file_name = "./checkpoint/ode/" + prefix + "test2.pth.tar"
    save_interval = 50
    plot_interval = 1
...
def construct_initial_state():
    ...
    def fun1():
        ...
        pertube_value2 = 2.0 * torch.tensor(gkern(7, 3))
        h0_true[70:77, 70:77] = pertube_value2
...
state.requires_grad = True
state0_true = state0_true.to(device)
...
if(args.load_model):
    state = load_checkpoint(torch.load(args.checkpoint_file_name))
    state.requires_grad = True

# pde model
pde_model = Shallow_Water_Model(grid_info=(dx, dy)).to(device)
t = torch.linspace(0., 0.3, 101).to(device)

# %%
y_true = odeint(pde_model, state0_true, t, method=args.method).to(device)
sig_b = 0.001
background_true = state0_true + sig_b * \
    torch.randn(state0_true.shape).to(device)
background_true[0, :, 0:25] = 0.0
background_true[0, :, 35::] = 0.0


# import matplotlib.pyplot as plt
# file_name = "background.png"
# plt.cla()
# plt.imshow(background_true[0, :, :].cpu().squeeze().detach().numpy())
...
print(y_true.shape)
# %%
# optimizer = torch.optim.Adam([state], lr=0.03)
state.requires_grad = True
optimizer = torch.optim.Adam([state], lr=0.1)
criterion = torch.nn.MSELoss()
obs_index = np.arange(50, 100, 10)
state_index = np.arange(0, 1, 1)
# x_index = range(5, 105, 10)
# y_index = range(5, 105, 10)
...
smoothing = GaussianSmoothing(
    channels=1, kernel_size=5, sigma=0.5, dim=2).to(device)

for iteration in range(args.niters):

    time_start = time.time()
    y_pred = odeint(pde_model, state, t, rtol=1e50, atol=1e50,
                    method=args.method, ).to(device)
    optimizer.zero_grad()
    loss1 = criterion(y_pred[obs_index, state_index, 25:35:1, 0:100:1],
                      y_true[obs_index, state_index, 25:35:1, 0:100:1])

    analysis_field = y_pred[0, state_index, :, :].unsqueeze(0)

    smooth_field = smoothing(analysis_field)

    # loss2 = criterion(smooth_field, analysis_field)
    # loss3 = criterion(
    #     analysis_field.squeeze()[0:100:1, 25:35:1], background_true[0, :, :].squeeze()[0:100:1, 25:35:1])

    # loss = loss1
    # loss = loss1 + 0.5 * loss2 + loss3
    loss = loss1

    # loss = criterion(y_pred[obs_index, state_index, :, :],
    #                  y_true[obs_index, state_index, :, :])
    loss.backward(retain_graph=True)
    optimizer.step()

    assimilation_loss = torch.mean(
        torch.abs(y_pred[0, 0, :, :] - y_true[0, 0, :, :]))

    writer.add_scalar("loss", loss.item(), global_step=iteration)
    logger.info("iteration: {:04d} | loss:{:.6f} | elapse time:{:.3f} | assimilation loss: {:.6f} ".format(
        iteration, loss.item(), time.time() - time_start, assimilation_loss.cpu().detach().numpy()))
    ...
    if (((iteration+1) % args.plot_interval == 0)):
        import matplotlib.pyplot as plt
        file_name = args.prefix + str(iteration+1).zfill(4) + ".png"
        plt.cla()
        plt.imshow(y_pred[0, 0, :, :].cpu().squeeze().detach().numpy())
        plt.savefig(os.path.join("results", args.main_folder,
                                 args.sub_folder, file_name))

        plt.cla()
        file_name = args.prefix + "u_" + str(iteration+1).zfill(4) + ".png"
        plt.imshow(y_pred[0, 1, :, :].cpu().squeeze().detach().numpy())
        plt.savefig(os.path.join("results", args.main_folder,
                                 args.sub_folder, file_name))

        plt.cla()
        file_name = args.prefix + "v_" + str(iteration+1).zfill(4) + ".png"
        plt.imshow(y_pred[0, 2, :, :].cpu().squeeze().detach().numpy())
        plt.savefig(os.path.join("results", args.main_folder,
                                 args.sub_folder, file_name))
