...
import numpy as np
import logging
from dynamic_model.differ_module import Order2_Diff1_Unstructure_Period, Order2_Diff2_Unstructure_Period,Order2_Diff1_Unstructure,Order2_Diff2_Unstructure
from torch.utils.tensorboard import SummaryWriter

import matplotlib.pyplot as plt

from tools.plot_helper import generate_image_movement_gif, generate_image_movement3d_gif, generate_image_movement2d_gif
...
class Shallow_Water_Model(nn.Module):
    def __init__(self, grid_info):
        ...
        if(True):
            # x = torch.arange(0,  101*dx, dx, dtype=torch.float32)
            ...
            self.diff2_y = Order2_Diff1_Unstructure_Period(
                self.vector_y, total_dim=2, diff_dim=2)
    ...
    def forward(self, t, state):
        ...
        du_adv = -self.diff1_x(u2) - self.diff1_y(uv)
        dv_adv = -self.diff1_x(uv) - self.diff1_y(v2)

        # y方向是周期条件，

        du = self.mu*(self.diff2_x(u) + self.diff2_y(u)) \
            - self.g*self.diff1_x(h) + du_adv - self.b*u
        dv = self.mu*(self.diff2_x(v) + self.diff2_y(v)) \
            - self.g * self.diff1_y(h) + dv_adv - self.b * v

        if(True):
            du[0, 0, :] = -(du[0, 0, :])*0.5
            du[0, self.nx - 1, :] = -du[0, self.nx - 1, :]*0.5

        if(False):
            du[0, 0, :] = -(du[0, 0, :] - du_adv[0, 0, :])
            du[0, self.nx - 1, :] = - \
                (du[0, self.nx - 1, :] - du_adv[0, self.nx - 1, :])
...
def construct_initial_state():
    ...
    x_temp = torch.arange(0,  101*dx, dx, dtype=torch.float32)
    y = torch.arange(0, 100 * dy, dy, dtype=torch.float32)
...
(dx, dy, grid_x, grid_y, vector_x,
 vector_y), state, state0_true = construct_initial_state()

if(False):
    import matplotlib.pyplot as plt
    from matplotlib.collections import LineCollection
    plt.scatter(grid_y[0:101:5, 0:100:5], grid_x[0:101:5, 0:100:5])
    segs1 = np.stack(
        (grid_y[0:101:5, 0:100:5], grid_x[0:101:5, 0:100:5]), axis=2)
...
# %%
pde_model = Shallow_Water_Model(grid_info=(
    dx, dy, grid_x, grid_y, vector_x, vector_y)).to(device)

t = torch.linspace(0., 0.3, 100).to(device)
# %%
...
# y_true = odeint(pde_model, state, t, method=args.method).to(device)

# if(False):
#     y_pred_repalce = torch.zeros(y_true.shape)
#     y_pred_repalce[:, 0, 25:75:1, :] = y_true[:, 0, 25:75:1, :]
#     y_true = y_pred_repalce


# generate_image_movement_gif(
#     y_true[:, 0, :, :].squeeze().detach().numpy(), "./result/shallow_wave_0830.gif", fps=10)


print(grid_x.shape, grid_y.shape, y_true[:, 0, :, :].squeeze(
).detach().numpy().shape)

generate_image_movement_gif(
    y_true[:, 0, :, :].squeeze().detach().numpy(), "./result/shallow_wave_0905_structure_debug.gif", fps=10)


generate_image_movement2d_gif(grid_x, grid_y, y_true[:, 0, :, :].squeeze(
).detach().numpy(), "./result/shallow_wave_0905_structure_debug2.gif", fps=10)
# generate_image_movement_gif(
