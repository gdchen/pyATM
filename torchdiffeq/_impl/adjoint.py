...
class OdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(ctx, shapes, func, y0, t, rtol, atol, method, options, event_fn, adjoint_rtol, adjoint_atol, adjoint_method,
        ...
        with torch.no_grad():
            ans = odeint(func, y0, t, rtol=rtol, atol=atol,
    ...
    def backward(ctx, *grad_y):
        with torch.no_grad():
            ...
            # vjp_t, y, vjp_y
            aug_state = [torch.zeros(
                (), dtype=y.dtype, device=y.device), y[-1], grad_y[-1]]
...
def odeint_adjoint(func, y0, t, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None,
    ...
    # def odeint_adjoint(func, y0, t, *, rtol=1.0, atol=1.0, method=None, options=None, event_fn=None,
    #                    adjoint_rtol=None, adjoint_atol=None, adjoint_method=None, adjoint_options=None, adjoint_params=None):
    ...
    if adjoint_options is None:
        adjoint_options = {k: v for k, v in options.items(
        ) if k != "norm"} if options is not None else {}
    else:
