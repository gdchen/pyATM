...
def _runge_kutta_step(func, y0, f0, t0, dt, t1, tableau):
    ...
    k = torch.empty(*f0.shape, len(tableau.alpha) + 1,
...
def rk4_alt_step_func(func, t0, dt, t1, y0, f0=None, perturb=False):
    ...
    # print(t0.device, y0.device, k1.device, t0.device)
    k2 = func(t0 + dt * _one_third, y0 + dt * k1 * _one_third)
    k3 = func(t0 + dt * _two_thirds, y0 + dt *
              (k2 - k1 * _one_third))
...
class RKAdaptiveStepsizeODESolver(AdaptiveStepsizeEventODESolver):
    ...
    def __init__(self, func, y0, rtol, atol,
                 ...
                 dtype=torch.float64,
                 **kwargs):
        super(RKAdaptiveStepsizeODESolver, self).__init__(
            dtype=dtype, y0=y0, **kwargs)
        ...
        self.first_step = None if first_step is None else torch.as_tensor(
            first_step, dtype=dtype, device=device)
        ...
        self.step_t = None if step_t is None else torch.as_tensor(
            step_t, dtype=dtype, device=device)
        self.jump_t = None if jump_t is None else torch.as_tensor(
            jump_t, dtype=dtype, device=device)
        ...
        self.tableau = _ButcherTableau(alpha=self.tableau.alpha.to(device=device, dtype=y0.dtype),
                                       beta=[b.to(device=device, dtype=y0.dtype)
                                             for b in self.tableau.beta],
                                       c_sol=self.tableau.c_sol.to(
                                           device=device, dtype=y0.dtype),
    ...
    def _before_integrate(self, t):
        ...
        self.step_t = step_t
        self.jump_t = jump_t
        self.next_step_index = min(bisect.bisect(
            self.step_t.tolist(), t[0]), len(self.step_t) - 1)
        self.next_jump_index = min(bisect.bisect(
            self.jump_t.tolist(), t[0]), len(self.jump_t) - 1)
    ...
    def _advance_until_event(self, event_fn):
        ...
        def interp_fn(t): return _interp_evaluate(
    ...
    def _adaptive_step(self, rk_state):
        ...
        assert torch.isfinite(
            y0).all(), 'non-finite values in state `y`: {}'.format(y0)
