...
class AdaptiveStepsizeODESolver(metaclass=abc.ABCMeta):
    ...
    def integrate(self, t):
        solution = torch.empty(len(t), *self.y0.shape,
                               dtype=self.y0.dtype, device=self.y0.device)
        solution[0] = self.y0
        t = t.to(self.dtype)
...
class FixedGridODESolver(metaclass=abc.ABCMeta):
        ...
        for t0, t1 in zip(time_grid[:-1], time_grid[1:]):
            ...
            while j < len(t) and t1 >= t[j]:
                if self.interp == "linear":
                    ...
                    raise ValueError(
                        f"Unknown interpolation method {self.interp}")
                j += 1
            y0 = y1
    ...
    def integrate_until_event(self, t0, event_fn):
        ...
        while True:
            ...
            if sign0 != sign1:
                if self.interp == "linear":
                    def interp_fn(t): return self._linear_interp(
                        t0, t1, y0, y1, t)
                elif self.interp == "cubic":
                    f1 = self.func(t1, y1)

                    def interp_fn(t): return self._cubic_hermite_interp(
                        t0, y0, f0, t1, y1, f1, t)
                else:
                    raise ValueError(
                        f"Unknown interpolation method {self.interp}")
            ...
            if itr >= max_itrs:
                raise RuntimeError(
                    f"Reached maximum number of iterations {max_itrs}.")
        solution = torch.stack([self.y0, y1], dim=0)
        return event_time, solution
